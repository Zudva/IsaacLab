...
REM Choose task for robot transfer tests:
REM   Isaac-Velocity-Flat-G1-EDU-v0  = flat terrain, basic locomotion
REM   Isaac-Velocity-Rough-G1-v0     = rough terrain, robust locomotion  
set TASK=Isaac-Velocity-Flat-G1-EDU-v0
set TARGET_VRAM_GB=14
set SAFETY_MB=300
set MAX_ITERS=20000
set SEED=42
REM Кандидаты для 14GB (по убыванию)
...
if defined CARB_DISABLE_MODULES (
  set "CARB_DISABLE_MODULES=%CARB_DISABLE_MODULES%;carb.graphics-vulkan.plugin;omni.kit.renderer.core.plugin.vulkan;omni.render.rtx.plugin.vulkan;omni.gpu_foundation_factory.plugin.vulkan"
) else (
...
for %%N in (%CAND_LIST%) do (
    ...
    REM Run quick test (1 iteration)
    ...
    if !RC! EQU 0 (
        if !TOTAL_USAGE! LEQ %TARGET_MB% (
            if !REMAINING! GEQ 0 (
                set BEST=%%N
                echo [ACCEPTED] %%N environments - VRAM usage: !TOTAL_USAGE!MB (!REMAINING!MB under target) >> "%LOG%"
            ) else (
                echo [REJECTED] %%N environments - Would exceed target by !REMAINING!MB >> "%LOG%"
                goto :found_best
            )
        ) else (
            echo [REJECTED] %%N environments - Exceeds %TARGET_VRAM_GB%GB target >> "%LOG%"
            goto :found_best
        )
    ) else (
        echo [FAILED] %%N environments - Training failed to start >> "%LOG%"
    )
...
:found_best
if %BEST% EQU 0 (
    echo [FALLBACK] No suitable environment count found, using 512 >> "%LOG%"
    set BEST=512
)
...
echo ===========================
echo.
echo [FINAL] Selected %BEST% environments for ~%TARGET_VRAM_GB%GB VRAM usage
echo [FINAL] Starting full training run...
echo.

REM Kill any remaining processes before final run
taskkill /f /im "python.exe" >nul 2>&1
powershell -NoLogo -NoProfile -Command "Start-Sleep -Seconds 3" >nul

REM Final training run
echo [TRAINING] %BEST% envs, %MAX_ITERS% iterations >> "%LOG%"
isaaclab.bat -p scripts\reinforcement_learning\rsl_rl\train.py --task %TASK% --num_envs %BEST% --max_iterations %MAX_ITERS% --seed %SEED% --headless

echo [END] %date% %time% >> "%LOG%"
echo Training completed. Check logs\rsl_rl\g1_edu\ for results.
pause
endlocal
